...
fn generate_zobrist_numbers() -> Result<()> {
    ...
    writeln!(f, "/// A zobrist number for when the current player is White")?;
    writeln!(f, "pub const ZOBRIST_WHITE_TURN: u64 = {};\n", rng.next_u64())?;

    let num_colors = 2;
    let num_pieces = 6;
    let num_positions = 64;
    let psc_count = num_colors * num_pieces * num_positions;
    writeln!(f, "/// A single array to hold all of the piece/square/color zobrist numbers")?;
    ...
    writeln!(f, "/// One zobrist number for each of the 16 possible castling rights combinations")?;
    ...
    writeln!(f, "/// One zobrist number for each file that could be en-passant")?;
